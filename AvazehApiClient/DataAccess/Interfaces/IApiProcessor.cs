...
namespace AvazehApiClient.DataAccess.Interfaces
{
    public interface IApiProcessor
    {
        ...
        Task<T> GetTransactionCollectionAsync<T>(string Key, string OrderBy, OrderType orderType, int Page = 1, string SearchText = "", InvoiceFinancialStatus? FinStatus = null, int PageSize = 50, bool ForceLoad = false) where T : class;
