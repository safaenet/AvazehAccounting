CREATE PROCEDURE [dbo].[LoadInvoiceList_OldStyle] 
	@FetchSize int = 50,
	@InvoiceId int = -1,
	@CustomerId int = -1,
	@Date dbo.DateType = '%',
...
(
	InvoiceId INT,
	PrevInvoiceId INT,
	PrevInvoiceSum dbo.CalculationType
)
...
(
	InvoiceId INT,
	PrevInvoiceId INT,
	PrevInvoicePays dbo.CalculationType
);
...
SELECT	TOP(@FetchSize)
		...
		InvFwds.fwdFactorNum AS FwdInvoiceId
		...
		AND
		((@LifeStatus != -1 AND i.LifeStatus = @LifeStatus) OR (@LifeStatus = -1))
		AND ((@InvoiceId > -1 AND (i.Id = @InvoiceId OR i.[PrevInvoiceId] = @InvoiceId OR InvFwds.fwdFactorNum = @InvoiceId)) OR (@InvoiceId = -1))
		...
		AND ((@FinStatus <> -1 AND ((@FinStatus = @BALANCED AND (SFS - ISNULL(PT, 0) + pib.PrevTotalBalance) = 0) OR 
			(@FinStatus = @DEPTOR AND (SFS - ISNULL(PT, 0) + pib.PrevTotalBalance) > 0) OR
			(@FinStatus = @CREDITOR AND (SFS - ISNULL(PT, 0) + pib.PrevTotalBalance) < 0) OR
			(@FinStatus = @OUTSTANDING AND InvFwds.fwdFactorNum IS NULL AND (SFS - ISNULL(PT, 0) + pib.PrevTotalBalance) <> 0)) OR
			@FinStatus = -1)
		)
