...
public class TransactionItemCollectionManager : ITransactionItemCollectionManager
{
    public TransactionItemCollectionManager(ITransactionProcessor processor)
    {
        Processor = processor;
    }
    ...
    public IEnumerable<TransactionItemModel> Items { get; set; }
    ...
    public int GenerateWhereClause(string val, string OrderBy, OrderType orderType, TransactionFinancialStatus? finStatus, string Date = null, bool run = false, SqlSearchMode mode = SqlSearchMode.OR)
    {
        if (val == SearchValue && OrderBy == QueryOrderBy && orderType == QueryOrderType) return 0;
        SearchValue = val;
        QueryOrderBy = OrderBy;
        QueryOrderType = orderType;
        WhereClause = Processor.GenerateTransactionItemWhereClause(val, finStatus, Date, mode);
    ...
    public async Task<int> GotoPageAsync(int PageNumber)
    {
        if (!Initialized)
        {
            TotalQueryCount = await Processor.GetTotalTransactionItemQueryCountAsync(WhereClause, TransactionId);
        ...
        Items = await Processor.LoadManyTransactionItemsAsync((PageNumber - 1) * PageSize, PageSize, WhereClause, TransactionId, QueryOrderBy, QueryOrderType);
