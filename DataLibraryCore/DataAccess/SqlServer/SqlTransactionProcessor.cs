...
public class SqlTransactionProcessor : ITransactionProcessor
{
    ...
    public async Task<IEnumerable<TransactionListDTO>> LoadManyItemsAsync(int OffSet, int FetcheSize, string WhereClause, string OrderBy = QueryOrderBy, OrderType Order = QueryOrderType)
    {
        try
        {
            ...
            return await DataAccess.LoadDataAsync<TransactionListDTO>(sql);
        }
        catch (Exception ex)
        {
            Log.Error(ex, "Error in SqlTransactionProcessor");
