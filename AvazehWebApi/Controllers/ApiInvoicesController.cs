...
public class InvoicesController : ControllerBase
{
    ...
    public async Task<ActionResult<List<InvoiceListDTO>>> GetPrevInvoicesAsync(int InvoiceId = -1, int CustomerId = -1, string InvoiceDate = "%", string SearchValue = "%", OrderType orderType = OrderType.DESC)
    {
        if (InvoiceId <= 0) InvoiceId = -1;
        if (CustomerId <= 0) CustomerId = -1;
